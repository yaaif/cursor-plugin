...
const clientSecretHeadersSchema = z.array(z.object({
 ...
 */
export function registerMcpDeploymentTools(server, ctx) {
    ...
    server.registerTool("yaaif_mcp_deployment_update", {
        ...
        try {
            const deployment = await ctx.api.apiJSON("GET", `/api/mcp-deployments/${encodeURIComponent(args.deployment_id)}`);
            const overrides = {};
            if (args.name !== undefined)
                overrides.name = args.name;
            if (args.image !== undefined)
                overrides.image = args.image;
            if (args.deployment_method !== undefined)
                overrides.deployment_method = args.deployment_method;
            if (args.container_port !== undefined)
                overrides.container_port = args.container_port;
            if (args.mcp_path !== undefined)
                overrides.mcp_path = args.mcp_path;
            if (args.endpoint_mode !== undefined)
                overrides.endpoint_mode = args.endpoint_mode;
            if (args.clear_endpoint_host)
                overrides.endpoint_host = null;
            else if (args.endpoint_host !== undefined)
                overrides.endpoint_host = args.endpoint_host;
            if (args.transport_type !== undefined)
                overrides.transport_type = args.transport_type;
            if (args.env !== undefined)
                overrides.env = args.env;
            if (args.secret_env !== undefined)
                overrides.secret_env = mapSecretEnv(args.secret_env);
            if (args.client_secret_headers !== undefined) {
                overrides.client_secret_headers = mapClientSecretHeaders(args.client_secret_headers);
            }
            if (args.auto_register !== undefined)
                overrides.auto_register = args.auto_register;
            if (args.auto_import_tools !== undefined)
                overrides.auto_import_tools = args.auto_import_tools;
            if (args.clear_registry_credential_id)
                overrides.registry_credential_id = null;
            ...
            return ok("Updated MCP deployment.", {
                deployment: await ctx.api.apiJSON("PUT", `/api/mcp-deployments/${encodeURIComponent(args.deployment_id)}`, body),
    ...
    server.registerTool("yaaif_mcp_deployment_redeploy", {
        ...
        try {
            return ok("Redeploy started.", {
                deployment: await ctx.api.apiJSON("POST", `/api/mcp-deployments/${encodeURIComponent(deployment_id)}/redeploy`, {}),
            });
        }
        catch (e) {
            return fail(String(e));
    ...
    server.registerTool("yaaif_mcp_deployment_stop", {
        ...
        try {
            return ok("Stop requested.", {
                deployment: await ctx.api.apiJSON("POST", `/api/mcp-deployments/${encodeURIComponent(deployment_id)}/stop`, {}),
            });
        }
        catch (e) {
            return fail(String(e));
    ...
    server.registerTool("yaaif_mcp_deployment_delete", {
        ...
        try {
            await ctx.api.apiJSON("DELETE", `/api/mcp-deployments/${encodeURIComponent(deployment_id)}?${params}`);
    ...
    server.registerTool("yaaif_mcp_deployment_k8s_status", {
        ...
        if (Object.keys(errors).length && Object.keys(out).length === 0) {
            return fail(`Kubernetes status unavailable (is DEPLOYMENT_KUBERNETES_ENABLED set?). ${Object.values(errors).join("; ")}`);
        }
