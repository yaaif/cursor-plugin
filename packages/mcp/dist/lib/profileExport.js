export function exportProfileEnv(cfg) {
    const vars = {
    ...
    if (cfg.defaultTenantId)
        vars.YAAIF_DEFAULT_TENANT_ID = cfg.defaultTenantId;
    if (cfg.extraCaFile)
        vars.YAAIF_EXTRA_CA_FILE = cfg.extraCaFile;
    if (cfg.clientCertFile)
        vars.YAAIF_CLIENT_CERT_FILE = cfg.clientCertFile;
    if (cfg.clientKeyFile)
        vars.YAAIF_CLIENT_KEY_FILE = cfg.clientKeyFile;
