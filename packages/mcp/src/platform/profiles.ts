...
export function deriveServiceUrls(apiBase: string) {
...
export function profileIdFromHostname(hostname: string): string {
  const slug = hostname
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
...
export function profileFromPlatformUrl(raw: string, oidcClientId = "yaaif-cursor"): PlatformProfile {
