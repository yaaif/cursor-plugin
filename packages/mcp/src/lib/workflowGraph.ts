...
export const ENGINE_SPINE_ONLY_WARNING =
  "workflow_graph is engine-spine-only (watcher/evaluator/guardian/orchestrator/recorder) with no step nodes. Author ACTION/MESSAGE/BRANCH/WAIT/HUMAN/TERMINAL steps instead; engine phases are settings, not canvas steps.";

export function graphNodes(graph: unknown): Array<{ id: string; type: string }> {
...
export function isEngineSpineOnlyGraph(graph: unknown): boolean {
