...
export function clampCatalogLimit(limit: number | undefined, fallback: number): number {
...
export function appendClampedLimit(params: URLSearchParams, limit: number | undefined): void {
...
export async function listAllMcpToolNames(
  getJSON: (path: string) => Promise<McpToolsPage>,
): Promise<string[]> {
  const names: string[] = [];
  let offset = 0;
