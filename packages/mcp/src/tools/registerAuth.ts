...
async function probeOidc(authority: string) {
...
async function loadMemberships(ctx: Ctx) {
  const raw = await ctx.api.apiJSON("GET", "/api/users/me/tenants");
  return parseTenantMemberships(raw);
}

async function loadLastTenantId(ctx: Ctx): Promise<string> {
  try {
...
async function activateTenant(ctx: Ctx, tenantId: string, tenantName?: string) {
...
async function resolveAndSetTenant(ctx: Ctx, query: string) {
  const memberships = await loadMemberships(ctx);
  const resolved = resolveTenant(memberships, query);
  if ("error" in resolved) {
    return { ok: false as const, ...resolved, memberships };
  }
...
async function autoSelectTenant(ctx: Ctx) {
  const memberships = await loadMemberships(ctx);
...
export function registerAuthTools(server: McpServer, ctx: Ctx): void {
  ...
  server.registerTool("yaaif_configure_check", {
    ...
    const probe = async (key: string, url: string) => {
      try {
  ...
  server.registerTool("yaaif_whoami", {
    ...
    const selected = Array.isArray(tenantsNorm)
      ? (tenantsNorm as { is_selected?: boolean; name?: string }[]).find((t) => t.is_selected)
      : undefined;
