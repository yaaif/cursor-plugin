...
export function registerLocalTools(server: McpServer, ctx: Ctx): void {
  ...
  server.registerTool("yaaif_local_tool_call", {
    ...
    try {
      ...
      const isError = Boolean((result as { is_error?: boolean })?.is_error);
      if (isError) {
  ...
  server.registerTool("yaaif_dev_session_ensure", {
    description:
      "Create or reuse a Cursor authoring chat session for files_* / session_state_* local tools. Persists session_id in ~/.yaaif/cursor/session.json.",
    inputSchema: {
      session_id: z.string().optional(),
      agent_id: z.string().optional(),
  ...
  const alias = (
    ...
    server.registerTool(tool, {
      ...
      const sessionId = await resolveDevSessionId(ctx, input.session_id as string | undefined);
      try {
