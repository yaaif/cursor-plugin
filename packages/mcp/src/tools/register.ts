...
function registerSkills(server: McpServer, ctx: Ctx): void {
  ...
  server.registerTool("yaaif_skill_get", {
    ...
    try {
      const result = await ctx.api.agentJSON<{ items?: unknown[] }>(
        "GET",
  ...
  server.registerTool("yaaif_skill_file_tree", {
    ...
    const path = `/api/skill-files/tree${params.size ? `?${params}` : ""}`;
    try {
      return ok("Fetched skill file tree.", { result: await ctx.api.agentJSON("GET", path) });
    } catch (e) { return fail(String(e)); }
  });
  ...
  server.registerTool("yaaif_skill_read_file", {
    ...
    try {
      const path = `/api/skill-files/content?path=${encodeURIComponent(filePath)}`;
  ...
  server.registerTool("yaaif_skill_map_agents_merge", {
    ...
    try {
      ...
      for (const a of assignments) {
        const agent = await ctx.api.agentJSON<{ skill_ids?: string[] }>(
          "GET",
...
function registerAmbient(server: McpServer, ctx: Ctx): void {
  ...
  server.registerTool("yaaif_ambient_agent_get", {
    ...
    inputSchema: { ambient_agent_id: z.string() },
  ...
  server.registerTool("yaaif_agent_update", {
    ...
    if (args.goal_prompt !== undefined) body.goal_prompt = args.goal_prompt;
    if (args.skill_ids !== undefined) body.skill_ids = args.skill_ids;
...
function registerMcp(server: McpServer, ctx: Ctx): void {
  ...
  server.registerTool("yaaif_catalog_overview", {
    description:
      "Read-only snapshot of the current tenant: agents, skills, MCP tools/servers/deployments, API keys, deployment settings status, ambient agents/workflows, local tools, file registry lifecycle (paginated summaries).",
    inputSchema: {
      q: z.string().optional(),
      limit: z.number().optional(),
    },
