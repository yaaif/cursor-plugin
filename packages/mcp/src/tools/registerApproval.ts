...
export function registerApprovalTools(server: McpServer, ctx: Ctx): void {
  ...
  server.registerTool("yaaif_approval_strategy_create", {
    ...
    try {
      const created = await ctx.api.approvalJSON<Record<string, unknown>>(
        "POST",
      ...
      if (args.publish) {
        const strategy = (created as { strategy?: { id?: string } }).strategy;
      ...
      return ok(`Created approval strategy ${args.name}.`, {
        created,
        published,
        approval_strategy_id: (created as { strategy?: { id?: string } }).strategy?.id,
      });
    } catch (e) { return fail(String(e)); }
  });
