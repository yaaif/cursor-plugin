...
async function resolveTenantUser(
  ...
  if (id) {
    return ctx.api.apiJSON<TenantUser>("GET", `/api/users/${encodeURIComponent(id)}`);
  }
  ...
  const page = await ctx.api.apiJSON<UsersPage>("GET", `/api/users?${params}`);
...
export function registerUserTools(server: McpServer, ctx: Ctx): void {
  ...
  server.registerTool("yaaif_user_role_set", {
    ...
    try {
      let current: TenantUser = { id: (user_id ?? "").trim() };
      if (current.id) {
        try {
      ...
      const user = await ctx.api.apiJSON<TenantUser>("PUT", `/api/users/${encodeURIComponent(id)}`, { role: targetRole });
