 ...
 */
export function registerApiKeyTools(server: McpServer, ctx: Ctx): void {
  ...
  server.registerTool("yaaif_api_key_create", {
    ...
    try {
      ...
      const result = await ctx.api.apiJSON<{
        key?: { id?: string; credential_id?: string };
        plaintext_key?: string;
        binding_hints?: unknown;
      }>("POST", "/api/mcp-platform-keys", body);
  ...
  server.registerTool("yaaif_api_key_update", {
    ...
    if (args.allowlists !== undefined) body.allowlists = args.allowlists;
  ...
  server.registerTool("yaaif_api_key_bind_deployment", {
    ...
    try {
      ...
      const mapping: SecretEnvMapping = {
        env_name: envName,
