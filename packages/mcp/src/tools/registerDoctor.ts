...
function tlsHint(err: unknown, tls = getTlsResolveInfo()): string | undefined {
...
export function registerDoctorTools(server: McpServer, ctx: Ctx): void {
  server.registerTool("yaaif_doctor", {
    ...
    const add = (name: string, okFlag: boolean, detail?: unknown) => {
    ...
    try {
      ...
      const doc = res.ok ? await res.json() as Record<string, unknown> : null;
    ...
    if (sessionReady) {
      ...
      try {
        const localTools = await ctx.api.agentJSON<{ count?: number; total?: number; family_counts?: Record<string, number> }>(
          "GET",
      ...
      try {
        const lifecycle = await ctx.api.agentJSON<{
          ...
          readiness?: {
            ready?: boolean;
            record_sink_wired?: boolean;
            grant_backend_wired?: boolean;
