...
```json
{ "command": "npx", "args": ["-y", "@yaaif/platform-mcp@1.3.4", "--client", "cursor"] }
```
...
3. Confirm `npm view @yaaif/platform-mcp version` reports `1.3.4` before relying on marketplace `npx --install`. The legacy `@yaaif/cursor-mcp` package remains a compatibility launcher. Root `mcp.json` already uses the npx pin.
